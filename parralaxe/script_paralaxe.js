...
var slider = function (idElement,largeurDiv,hauteurDiv,vitesseSlide,modeSlide,timeInterval) {
	...
	var backgroud = idElement + ">img:first-of-type";
	var compteurbackground = 0;
	var deplacementbackground = ($(idElement).width()/2) / (countImageOnSlider);
	console.log(deplacementbackground);
	console.log($(idElement).width());
	console.log(countImageOnSlider);
	var v;
	var k = 0;
	...
	slider.prototype.slide = function(){
		...
		v = setInterval(function(){
	          ...
	          if(compteurbackground < countImageOnSlider-1) {
				compteurbackground++;
				$(backgroud).animate({left:deplacementbackground*compteurbackground + "px"},vitesseSlide);
			}
			else{}
	          if(k == 5) { k = 0; }
				else { k++; }
	...
	slider.prototype.action = function(){
		$("#right").click(function(){
			...
			if(compteurbackground <= countImageOnSlider-1 && compteurbackground >= 0) {
				
				$(backgroud).animate({left:deplacementbackground*compteurbackground + "px"},vitesseSlide);
				console.log(deplacementbackground*compteurbackground);
			}
			else {}
			if(k == 5) { k = 0; }
			else { k++; }
		...
		$(idElement+">button").click(function(){
			...
			if(l < 0) {
				...
				$(backgroud).animate({left:deplacementbackground*b + "px"},vitesseSlide);
				compteurbackground = b;
			slider.prototype.slide();
		});
	}
	...
	slider.prototype.css = function() {
		...
		$(idElement).css({overflow: "hidden", position: "relative"});
		$(backgroud).css({zIndex: -50, position: "absolute", left: "0px",minWidth:$(idElement).width() * 2 + "px", marginLeft: -$(idElement).width()});
...
var hero = new slider ("#element",1000,300,2000,"auto",5000);
hero.getVariable();
hero.setSizeOfDiv();
hero.resize();
hero.setButton();
